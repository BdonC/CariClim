...
figsizes={
  ...
  'JAM':{'bias_correction':(6,5),'detail':(9,5)},
...
for country in COU.keys():
  for scenario in ['rcp45']:
    for indicator in indicator_dict.keys():
      seasons=COU[country]._seasons.keys()
      #cordex=COU[country].selection([indicator,scenario,'CORDEX_BC','ensemble_mean'])[0]
      #if cordex.time_format=='monthly':   seasons=COU[country]._seasons.keys()
      #if cordex.time_format=='10day':   seasons=COU[country]._seasons.keys()
      #if cordex.time_format=='yearly':    seasons=['year']
      ...
      #ewembi=COU[country].selection([indicator,'EWEMBI'])[0]
      print country
      print COU[country]._masks.keys()
      
      for region in COU[country]._masks['360x720_lat_-89.75_89.75_lon_-179.75_179.75']['lat_weighted'].keys():
        for season in seasons:
